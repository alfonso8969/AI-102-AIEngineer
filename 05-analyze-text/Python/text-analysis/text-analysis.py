...
from azure.ai.textanalytics import (AnalyzeSentimentResult,
                                    DetectLanguageResult, DocumentError,
                                    TextAnalyticsClient)
# Import namespaces
...
def main() -> None:
    try:
        ...
        for file_name in os.listdir(reviews_folder):
            ...
            # Get language
            detected_language: DetectLanguageResult | DocumentError = \
                cog_client.detect_language(documents=[text])[0]
            print(f"\nLanguage: {detected_language.primary_language.name}")
            # Get sentiment
            sentiment_analysis: AnalyzeSentimentResult | DocumentError = \
                cog_client.analyze_sentiment(documents=[text])[0]
